...
library StorageSlotExtension {
    ...
    /// @dev Load the value held at location `slot` in transient storage.
    function tload(Uint256SlotType slot) internal view returns (uint value) {
        /// @solidity memory-safe-assembly
        assembly {
            value := tload(slot)
        }
    ...
    /// @dev Store `value` at location `slot` in transient storage.
    function tstore(Uint256SlotType slot, uint value) internal {
        /// @solidity memory-safe-assembly
        assembly {
            tstore(slot, value)
        }
    ...
    /// @dev Load the value held at location `slot` in transient storage.
    function tload(Int256SlotType slot) internal view returns (int value) {
        /// @solidity memory-safe-assembly
        assembly {
            value := tload(slot)
        }
    ...
    /// @dev Store `value` at location `slot` in transient storage.
    function tstore(Int256SlotType slot, int value) internal {
        /// @solidity memory-safe-assembly
        assembly {
            tstore(slot, value)
        }
