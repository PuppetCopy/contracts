...
abstract contract Permission {
    ...
    mapping(address => mapping(bytes4 methodSignature => bool)) public permissionMap;

    function canCall(address user, bytes4 methodSignature) public view returns (bool) {
        return permissionMap[user][methodSignature];
    }
