...
interface IGmxReferralStorage {
    ...
    function setTier(uint _tierId, uint _totalRebate, uint _discountShare) external;
    // @dev set the tier for an affiliate
    // @param _tierId the tier level
    function setReferrerTier(address _referrer, uint _tierId) external;
