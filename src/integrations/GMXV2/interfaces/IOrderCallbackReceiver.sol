...
interface IOrderCallbackReceiver {
    ...
    function afterOrderExecution(bytes32 key, IGMXOrder.Props memory order, bytes memory eventData) external;

    /// @dev called after an order cancellation
    /// @param key the key of the order
    /// @param order the order that was cancelled
    function afterOrderCancellation(bytes32 key, IGMXOrder.Props memory order, bytes memory eventData) external;
    ...
    function afterOrderFrozen(bytes32 key, IGMXOrder.Props memory order, bytes memory eventData) external;
}
